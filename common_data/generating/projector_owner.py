...
f  = open('projector_serial_number.dat','r')
f2 = open('add_projector_owner.sql','w')
f3 = open('delete_projector_owner.sql','w')
for eq in range(0, 230):
    # вернули снилсы товарищей, которые могли пользоваться этим оборудованием
    req = "/maybe_owners/" + ''.join(str(linecache.getline('projector_serial_number.dat', eq+1)).splitlines()) + "/"
    c.request("GET", req)
    ...
    for i in range( num ):
        ...
        req = "/owners/" + ''.join(str(linecache.getline('projector_serial_number.dat', eq+1)).splitlines()) + "/" + owners[ind].replace(' ', '%20') + "/"
        c.request("GET", req)
        dates = c.getresponse() 
        # переписать в массив
        ...
        add_eq_owner  += '\'' + ''.join(str(linecache.getline('projector_serial_number.dat', eq+1)).splitlines()) + '\', '
        add_eq_owner  += '\'' + str(dates[0])                                                                   + '\' '
        add_eq_owner  += ');' + '\n'

        del_eq_owner  =  'CALL delete_equipment_owner( '
        del_eq_owner  += '\'' + owners[ind]                                                                     + '\', '
        del_eq_owner  += '\'' + ''.join(str(linecache.getline('projector_serial_number.dat', eq+1)).splitlines()) + '\', '
