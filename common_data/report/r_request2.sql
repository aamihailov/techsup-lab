...
SELECT tmp.snils,
       tmp.name,
       (
        SELECT _techsup_right2.get_sum_task_for_employee_priority(tmp.snils, 'самый низкий' )
       ) AS "P1",
       (
        SELECT _techsup_right2.get_sum_task_for_employee_priority(tmp.snils, 'низкий' )
       ) AS "P2",
       (
        SELECT _techsup_right2.get_sum_task_for_employee_priority(tmp.snils, 'средний' )
       ) AS "P3",
       (
        SELECT _techsup_right2.get_sum_task_for_employee_priority(tmp.snils, 'высокий' )
       ) AS "P4",
       (
        SELECT _techsup_right2.get_sum_task_for_employee_priority(tmp.snils, 'самый высокий' )
       ) AS "P5"
FROM _techsup_right2.employee
RIGHT JOIN (
  ...
  WHERE employee.role_id = (
            ...
            WHERE LOWER( employee_role.name ) = 'техник'
  ) 
--  LIMIT 10
